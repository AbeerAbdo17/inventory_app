...
public class VoucherForm1 extends JFrame {
    ...
    public VoucherForm1() {
       ...
       ImageIcon saveIcon = resizeIcon("save-32.png", 32, 32);
       ImageIcon deleteIcon = resizeIcon("trash-2-32.png", 32, 32);
       ImageIcon backIcon = resizeIcon("power-32.png", 32, 32);           
        ...
        JLabel searchIcon = new JLabel(resizeIcon("search-3-32.png", 20, 20));
...
//         new Frmmain().setVisible(true); 
    ...
    try (Connection conn = acc.Connect.ConnectDB();
   ...
   private void resetForm() {
    ...
    private int fetchTopNoFromYearTopNo() {
        ...
        try (Connection conn = acc.Connect.ConnectDB();
             ...
             ResultSet rs = stmt.executeQuery("SELECT MAX(topno) AS max_no FROM yeartopno ")) {//where SYSYEAR = '"+Frmlogin.sysYear+"'"
    ...
    private void insertTopNo(int topno) {
        try (Connection conn = acc.Connect.ConnectDB()) {
            PreparedStatement ps = conn.prepareStatement("INSERT INTO yeartopno (topno, sysyear) VALUES (?)");//add ?
            ps.setInt(1, topno);
//            ps.setInt(2, Frmlogin.sysYear);
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
...
public void printReport() {
    try {
        ...
        String reportPath = "/voucherapp/RepVouchar.jasper";

        HashMap<String, Object> parameters = new HashMap<>();
        parameters.put("Voucher_date", dateField.getText());     
        parameters.put("VoucherNo", currentVoucherNo);            
        parameters.put("Voucher_ref", referenceField.getText());   
        ...
        e.printStackTrace();
        JOptionPane.showMessageDialog(this, "Report Printing error" + e.getMessage());
    }
}
...
//if (!inputDate.contains(String.valueOf(Frmlogin.sysYear))) {
...
//     } 
        ...
        try (Connection conn = acc.Connect.ConnectDB()) {
            ...
            PreparedStatement ps = conn.prepareStatement("INSERT INTO journal (JOURNAL_NO, JOURNAL_SUBMAIN_NO, JOURNAL_DR, JOURNAL_CR, JOURNAL_DESC, JOURNAL_DATE, JOURNAL_DOCNO, journal_user) VALUES (?, ?, ?, ?, ?, ?, ?)");// add ?
    ...
    /** Filter vouchers based on search text */
     ...
     try (Connection conn = acc.Connect.ConnectDB()) {
         ...
         String currentYear = "2025";//String.valueOf(Frmlogin.sysYear);
...
System.out.println("Matched JOURNAL_DATE: " + rs1.getString("JOURNAL_DATE"));
       ...
       PreparedStatement ps = conn.prepareStatement(
    "SELECT j.JOURNAL_SUBMAIN_NO, s.SUBMAIN_NAME, j.JOURNAL_DR,journal_docno, j.JOURNAL_CR, j.JOURNAL_DESC " +
    "FROM journal j " +
    "LEFT JOIN submain s ON j.JOURNAL_SUBMAIN_NO = s.SUBMAIN_NO " +
...
ps.setString(3, currentYear);
    ...
    private boolean isAccExists(String acc1) throws SQLException {
        Connection conn = acc.Connect.ConnectDB();
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new VoucherForm1().setVisible(true));
    }
}
